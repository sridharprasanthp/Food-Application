...
export default function Tbody5(props) {
  ...
  let breakfasttotalcount = order.filter((dat) => {
    return (new Date(dat.date).getMonth() + 1 === new Date(props.filter).getMonth() + 1) && dat.breakfast === "true"&& (new Date(dat.date).getDate() >= 26) 
  })

  let lunchtotalcount = order.filter((dat) => {
    return (new Date(dat.date).getMonth() + 1 === new Date(props.filter).getMonth() + 1) && dat.lunch === "true"&& (new Date(dat.date).getDate() >= 26) 
  })
  let dinnertotalcount = order.filter((dat) => {
    return (new Date(dat.date).getMonth() + 1 === new Date(props.filter).getMonth() + 1) && dat.dinner === "true"&& (new Date(dat.date).getDate() >= 26) 
  })
  ...
  const last5days = getDaysInMonth(currentYear, currentMonth);

  for (var i = 26; i <= last5days; i++) {
        ...
        {props.datas.filter((gets) => gets._id !== "638041665140bfbdbc1f5d4a").map((userdata) => <>
          ...
          <tr key={userdata._id} style={{ borderBottom: "1px solid black" }}>
            ...
            <td className="text-center text-dark fw-bold" style={{ borderRight: "1px solid black" }}>
              {userdata.date.map((userbreakfast) => { (new Date(userbreakfast.date).getMonth() + 1 === new Date(props.filter).getMonth() + 1) && userbreakfast.breakfast === "true" && (new Date(userbreakfast.date).getDate() >= 26) ? <>{brkf++}</> : <></> }
              )}
              {brkf === null ? <p style={{ textAlign: "center" }}>-</p> : <p style={{ textAlign: "center" ,paddingTop:"4px"}}>{brkf}</p>}
            </td>

            <td className="text-center text-dark fw-bold" style={{ borderRight: "1px solid black" }}>
              {userdata.date.map((userbreakfast) => { (new Date(userbreakfast.date).getMonth() + 1 === new Date(props.filter).getMonth() + 1) && userbreakfast.lunch === "true"&& (new Date(userbreakfast.date).getDate() >= 26) ? <>{lunc++}</> : <></> }
              )}
              {lunc === null ? <p style={{ textAlign: "center" }}>-</p> : <p style={{ textAlign: "center" ,paddingTop:"4px"}}>{lunc}</p>}
            </td>

            <td className="text-center text-dark fw-bold" style={{ borderRight: "1px solid black" }}>
              {userdata.date.map((userbreakfast) => { (new Date(userbreakfast.date).getMonth() + 1 === new Date(props.filter).getMonth() + 1) && userbreakfast.dinner === "true" && (new Date(userbreakfast.date).getDate() >= 26)? <>{dinn++}</> : <></> }
              )}
